...
describe 'UpdateAccount', ->
  ...
  it "updates the user fields", (done)->
    u = new User name: 'Mah name', email: 'mah email', plan: 'free'
    u.save (err)->
      expect(err).to.equal(null)
      params = {name: 'New Name', email: 'new email', plan: 'enterprise'}
      session = {user: u}
  ...
  describe "won't overwrite with blank values", ->
    ...
    it "won't overwrite with blank plan", (done)->
      u = new User name: 'Mah name', email: 'mah email', plan: 'free'
      u.save (err)->
        expect(err).to.equal(null)
        params = {name: 'New Name', plan: ''}
        session = {user: u}
        callback = (err, response)->
          expect(err).to.equal(null)
          expect(response.name).to.equal('New Name')
