...
describe 'EnsureStripeCustomerOnAccount', ->
  ...
  it 'does not call to stripe if the account has a stripeCustomer', (done)->
    account = new Account(billingProvider: 'cine.io', productPlans: {broadcast: ['pro']}, billingEmail: 'the email', name: 'Chillin', stripeCustomer: {stripeCustomerId: 'da-stripe-customer'})
